...
class HCPMovieELTrialLabeling(Trial):
    ...
    def __init__(self, session, trial_nr, phase_durations,parameters, phase_names=None, timing='seconds',verbose=True):
        """ Initializes a StroopTrial object. 
        ...
        for text in self.text_list:
            self.text_stims.append(TextStim(self.session.win, text=text, height=txt_height, wrapWidth=txt_width, color='white'))
    ...
    def get_events(self): #record grading of movie #每个视频下面又有一个dict，里面是每个phase/text的评分 改grade记录 phase time随着信号结束  视频源
        #trail waiting for events to stop
        events = Trial.get_events(self)
        if events:
