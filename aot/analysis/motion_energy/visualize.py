...
def load_results(segmentation, aot_condition, slice_nr, subject, results_type):
    try:
        ...
        print(
            f"loading {results_type} for {segmentation}, slice {slice_nr} - shape: {results.shape}",
            flush=True,
        )
        return results
    except FileNotFoundError:
...
def average_voxel_params(filters, betas):
    ...
    print(f"selected_betas: {np.min(selected_betas)} - {np.max(selected_betas)}")
...
def concat_slices(filters, segmentation, aot_condition, n_voxels, n_slices, subject):
    ...
    for slice_nr in range(n_slices):
        ...
        betas = load_results(segmentation, aot_condition, slice_nr, subject, "betas")
...
def main(
    ...
    params, r2 = concat_slices(
        filters, segmentation, aot_condition, n_voxels, n_slices, subject
    )
    ...
    for param_idx, param_label in enumerate(param_names):
        print(f"Saving nifti for param {param_label}", flush=True)
        io_utils.save_nifti(
            params[:, :, :, param_idx],
