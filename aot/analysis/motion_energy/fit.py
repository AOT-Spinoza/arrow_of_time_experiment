...
def load_r2(subject):
    ...
    return nib.load(
        DIR_DERIVATIVES
        / f"sub-{subject}/GLMsingle_analysis/sub-{subject}_R2_aot_mean.nii.gz"
    ).get_fdata()
...
def load_amplitudes(subject, session, segmentation):
    ...
    glmsingle = np.load(
        DIR_DERIVATIVES
        / f"sub-{subject}/ses-{session}/GLMsingle_{segmentation}/TYPED_FITHRF_GLMDENOISE_RR.npy",
        allow_pickle=True,
    ).item()
...
def create_trial_sequence(subject, session, aot_condition=None):
    ...
    for run_idx in range(10):
        with open(
            DIR_SETTINGS
            / f"experiment_settings_sub_{str(subject).zfill(2)}_ses_{str(session).zfill(2)}_run_{str(run_idx + 1).zfill(2)}.yml"
        ) as f:
            run_trial_sequence = yaml.load(f, Loader=yaml.FullLoader)["stimuli"][
                "movie_files"
            ]
        run_trial_sequence = [label for label in run_trial_sequence if label != "blank"]
...
def select_voxels(subject, amplitudes, n_slices, slice_nr, r2, rsq_threshold=None):
    ...
    if rsq_threshold is not None:
        ...
        selected_ids = np.where(~(np.isnan(amplitudes).any(axis=1)))[0]
    ...
    np.save(out_path / f"vertices_slice_{str(slice_nr).zfill(4)}.npy", slice_vertices)
...
def create_design_matrix(motion_energy, trial_labels, subject, session):
    ...
    np.save(
        ...
        / f"sub-{subject}"
        / f"ses-{session}"
        / f"sub-{subject}_ses-{session}_motion_energy_design_matrix.npy",
        design_matrix,
    )
...
def evaluate(model, X, y):
    ...
    print(f"evaluating - shape y_pred: {y_pred.shape}, shape y_true: {y.shape}")
    print(
...
def save_pipeline(model, r2, subject, segmentation, aot_condition, slice_nr):
    subject = str(subject).zfill(3)
    path = (
        DIR_DERIVATIVES / f"sub-{subject}" / f"fracridge_{segmentation}_{aot_condition}"
    )
    ...
    with open(path / f"{base_filename}_model.pkl", "wb") as f:
        pickle.dump(model, f)
    np.save(path / f"{base_filename}_betas.npy", model.coef_)
    np.save(path / f"{base_filename}_alphas.npy", model.alpha_)
...
def get_data(subject, n_slices, slice_nr, segmentation, aot_condition, rsq_threshold):
    ...
    for session in range(1, 6):
        ...
        print(
            f"Shapes before transformations - motion energy: {motion_energy.shape} and amplitudes: {session_amplitudes.shape}",
            flush=True,
        )
        ...
        print(
            "Number of negative motion energy values:",
        ...
        print(
            f"Number of nans - dm: {np.isnan(session_design_matrix).sum()}, amplitudes: {np.isnan(session_amplitudes).sum()}",
            flush=True,
        )
        session_amplitudes = np.nan_to_num(session_amplitudes)  # replace nan by zero

        print(
            f"Shapes after transformations - design: {session_design_matrix.shape}, amplitudes: {session_amplitudes.shape}",
            flush=True,
        )
...
def main(
    ...
    save_pipeline(model, r2, subject, segmentation, aot_condition, slice_nr)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-sub", "--subject", type=int, default=1, help="Subject number."
    )
    parser.add_argument(
        "-slice", "--slice_nr", type=int, default=1, help="Slice number."
    )
    parser.add_argument("--n_jobs", type=int, default=2, help="Number of jobs")
