...
def index_to_bold_file(task, run):  # string,string
    template = "/tank/shared/2022/arrow_of_time/derivatives/fmripreps/aotpilot_preprocs/preproc7/sub-001/ses-pilot/func/sub-001_ses-pilot_task-72_acq-nordic_run-01_space-T1w_desc-preproc_bold.nii.gz"
    # construct the path of the bold file for the current task and run(do not use replace and the template!)
    test_bold_file = (
        "/tank/shared/2022/arrow_of_time/derivatives/fmripreps/aotpilot_preprocs/preproc7/sub-001/ses-pilot/func/sub-001_ses-pilot_task-"
        + task
        + "_acq-nordic_run-"
        + run
        + "_space-T1w_desc-preproc_bold.nii.gz"
    )
    return test_bold_file


def index_to_events_file(task, run):
    # template = '/tank/shared/2022/arrow_of_time/bids_data/aotpilot_final/sub-001/ses-pilot/func/sub-001_ses-pilot_task-72_run-01_events.tsv'
    events_file = (
        "/tank/shared/2022/arrow_of_time/bids_data/aotpilot_final/sub-001/ses-pilot/func/sub-001_ses-pilot_task-"
        + task
        + "_run-"
        + run
        + "_events.tsv"
...
def proc_bold_data(test_bold_file):
    img = nib.load(test_bold_file)
    print("img shape:", img.shape)
    # img_data = [x.data for x in img.darrays]
    # cur_data = np.array(img_data)#[0]
    # cur_data = cur_data[5:,:]   #first five
    # cur_data = cur_data[:-5,:] #last five
    # swap the first two dimensions
    # cur_data = np.swapaxes(cur_data,0,1)
    cur_data = img.get_fdata()
    # truncate the first time points
    cur_data = cur_data[:, :, :, 5:]
    print("bold data shape:", cur_data.shape)
    return cur_data  # [x,y,z,time]
...
def construct_bold_data_for_all_runs(task):
    bold_data_all_runs = []
    for run in [1, 2]:
        test_bold_file = index_to_bold_file(task, str(run).zfill(2))
        bold_data_one_run = proc_bold_data(test_bold_file)
        bold_data_all_runs.append(bold_data_one_run)
    return bold_data_all_runs


def construct_output_dir(task):
    output_dir_base = "/tank/shared/2022/arrow_of_time/arrow_of_time_exp/aot/analysis/glmsingle/outputs/pilot/outputon7_voxels_withfigure"
    output_dir = os.path.join(output_dir_base, task)
    return output_dir


def construct_figure_dir(task):
    figure_dir_base = "/tank/shared/2022/arrow_of_time/arrow_of_time_exp/aot/analysis/glmsingle/outputs/pilot/outputon7_voxels_withfigure"
    figure_dir = os.path.join(figure_dir_base, task)
    return figure_dir
...
def condition_order_list_for_all():
    def index_list_for_all_tasks():
        ...
        index_list_all_tasks = [
            item for sublist in index_list_all_tasks for item in sublist
        ]
        index_list_all_tasks = [
            item for sublist in index_list_all_tasks for item in sublist
        ]
...
def apply_glmsingle_for_one_task(task):
    ...
    figure_dir = construct_figure_dir(task)
    opt = dict()
    ...
    glmsingle_obj.fit(
        design=design_all_runs,
        data=bold_data_all_runs,
        stimdur=2.5,
        tr=1.6,
...
def apply_glmsingle_for_all_together():
    ...
    figure_dir = construct_figure_dir("all")
    opt = dict()
    opt["wantlibrary"] = 1
    opt["wantglmdenoise"] = 1
    opt["wantfracridge"] = 1
