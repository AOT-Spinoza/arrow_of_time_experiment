...
def index_to_bold_file(task,run): #string,string
    template = '/tank/shared/2022/arrow_of_time/preproc7/sub-001/ses-pilot/func/sub-001_ses-pilot_task-72_acq-nordic_run-01_space-T1w_desc-preproc_bold.nii.gz'
    #construct the path of the bold file for the current task and run(do not use replace and the template!)
    test_bold_file = '/tank/shared/2022/arrow_of_time/preproc7/sub-001/ses-pilot/func/sub-001_ses-pilot_task-'+task+'_acq-nordic_run-'+run+'_space-T1w_desc-preproc_bold.nii.gz'
...
def construct_output_dir(task):
    output_dir_base = '/tank/shared/2022/arrow_of_time/arrow_of_time/aot/analysis/glmsingle/outputon7_voxels'
...
if __name__ == '__main__':
    ...
    task = '72'
    run = '01'
    events_file = index_to_events_file(task,run)
    design,index_list = construct_design_for_one_run(events_file)
    print(design.shape)
    print(index_list)
    bold_file = index_to_bold_file(task,run)
    bold_data = proc_bold_data(bold_file)
    '''
