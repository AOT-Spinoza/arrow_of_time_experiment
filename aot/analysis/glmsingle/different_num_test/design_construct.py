...
def movie_conditions_dict():#include blank condition as 0
    ...
    resampled_video_names = ["S_"+name for name in original_video_names]#for gradin and labeling
...
def index_to_exp_yml(sub,ses,run):
    ...
    ses = str(ses).zfill(2)
    run = str(run).zfill(2)
    target_path = settings_root_path / "main" / f"experiment_settings_sub_{sub}_ses_{ses}_run_{run}.yml"
    return target_path

def construct_design_for_one_session(sub,ses,number_of_runs):
    list_of_designs = []
    run_number = number_of_runs
...
def construct_bold_for_one_session(sub,ses,datatype,number_of_runs):#fsnative or §T1W
    list_of_bold_data = []
    run_number = number_of_runs
...
def merge_bold_data(test_bold_file_L,test_bold_file_R): #directly from old, maybe need some change
...
def construct_output_dir(sub,ses,number_of_runs):
    output_dir = output_root + "/" +str(number_of_runs) + "/" +'sub-'+str(sub).zfill(3)+'_ses-'+str(ses).zfill(2)
...
def apply_glmsingle_for_one_session(sub,ses,number_of_runs):
    bolds = construct_bold_for_one_session(sub,ses,datatype='T1W',number_of_runs=number_of_runs)
    designs = construct_design_for_one_session(sub,ses,number_of_runs=number_of_runs)
    output_dir = construct_output_dir(sub,ses,number_of_runs=number_of_runs)
    opt = dict()
    ...
    glmsingle_obj.fit(design=designs,data=bolds,stimdur=2.5,tr=0.9,outputdir=output_dir)
...
if __name__ == '__main__':
    ...
    #print(len(bold_list))
    list_of_runs_number = [2,3,4,5,6,7,8,9,10]
    for number_of_runs in list_of_runs_number:
        apply_glmsingle_for_one_session(sub=2,ses=1,number_of_runs=number_of_runs)
